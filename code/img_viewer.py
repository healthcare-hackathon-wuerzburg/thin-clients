...
def convert_to_bytes(file_or_bytes, resize=None):
    """
    Convert image file or bytes to bytes format.

    Parameters:
        file_or_bytes (str or bytes): File path or bytes representing the image.
        resize (tuple, optional): Tuple specifying the new width and height for resizing the image.

    Returns:
        bytes: Converted image in bytes format.
    """
...
def main() -> None:
    """
    Main function to run the PySimpleGUI application.

    Displays an image viewer window with the option to evaluate images and show analysis results.
    """
    # --------------------------------- Define Layout ---------------------------------
